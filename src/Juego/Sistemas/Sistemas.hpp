...
namespace IVJ
{
    ...
    [[maybe_unused]] void AdjustEntityStats(std::shared_ptr<CE::Objeto> entity, int type);
    [[maybe_unused]] void SystemFollowPlayer(std::vector<std::shared_ptr<CE::Objeto>>& chasers, CE::Objeto& target, float dt);
    [[maybe_unused]] std::vector<std::shared_ptr<CE::Objeto>>& getEntityTypeVector(std::vector<std::shared_ptr<CE::Objeto>>& entities, CE::ENTITY_TYPE entityType);
