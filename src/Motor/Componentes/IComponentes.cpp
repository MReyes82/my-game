...
namespace CE
{
    ...
    ITimer::ITimer()
        :IComponentes{},frame_actual{0}, max_frame{0}
    {}

    ITimer::ITimer(int max_f)
        :IComponentes{},frame_actual{0}, max_frame{max_f}
    {}
    ...
    IRespawn::IRespawn(std::vector<std::shared_ptr<IComponentes> > &lista_comp, int max)
        : timer_actual{0}, timer_maximo{max}, componentes{lista_comp}, num_objetos{0}, width{0}, height{0}, MAX_ENTIDADES{10}
    {
        prefab = nullptr;
    }

    IRespawn::IRespawn(int max, int w, int h, IVJ::Entidad *pre, int max_entidades)
        : timer_actual{0}, timer_maximo{max}, width{w}, height{h},num_objetos{0}, MAX_ENTIDADES{max_entidades}
    {
        prefab = pre;
    }
